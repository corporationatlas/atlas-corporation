      ...
      <div className="relative w-full pt-[70%] bg-[#0d0e12] overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5 z-10">
          ...
          {vehicle.badge && (
            <span className="px-2 py-0.5 text-[10px] font-bold bg-red-600/90 text-white rounded-md w-fit">
              {language === 'es' ? vehicle.badge : t('vehicles.mostRequested')}
            </span>
          )}
        </div>
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-4">
          ...
          >
            ...
            <span>{t('vehicles.viewDetails')}</span>
          </button>
        </div>
      </div>
        ...
        <div className="pt-3 border-t border-white/10 flex items-center justify-between gap-3">
          <div>
            ...
            <span className="text-[10px] text-emerald-400 font-medium flex items-center gap-1">
              <ShieldCheck className="w-3 h-3" /> {language === 'es' ? 'Puesto en Venezuela' : 'Turnkey in Venezuela'}
            </span>
          </div>
          ...
          >
            {isAdding ? (
              <>
                ...
                <span>{language === 'es' ? 'Agregado' : 'Added'}</span>
              </>
            ) : (
              <>
                <Plus className="w-3.5 h-3.5" />
                <span>{language === 'es' ? 'Cotizar Procura' : 'Quote Procurement'}</span>
              </>
            )}
          </button>
        </div>
